...
async function googleError(response: Response, fallback: string) {
  ...
  try {
    ...
    // Keep the sanitized fallback below. Never surface token responses or HTML bodies.
  }
  return fallback;
}
...
export async function getGoogleSheetsAccessToken() {
  ...
  if (!oidcToken) {
    throw new Error("Vercel OIDC token is unavailable. This Google Sheets path must run in an OIDC-enabled Vercel environment.");
  }
