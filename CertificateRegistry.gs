...
const CERTIFICATE_REGISTRY_HEADERS_ = [
  "Certificate_ID", "Type", "Name", "Address", "Position", "Subject", "Reference",
...
function ensureCertificateRegistrySheet_() {
  ...
  const existing = sheet.getRange(1, 1, 1, width).getDisplayValues()[0].map(function(value) {
...
function certificateRegistryRows_(sheet) {
  ...
  return values.map(function(row, index) {
    ...
    headers.forEach(function(header, column) {
      if (header) record[header] = row[column] === undefined ? "" : row[column];
    });
    return record;
  }).filter(function(record) {
    return String(record.Certificate_ID || "").trim();
  });
}

function certificateRegistryClean_(value, maxLength) {
  return String(value == null ? "" : value).trim().replace(/\s+/g, " ").slice(0, maxLength || 900);
}
...
function certificateRegistryFind_(sheet, certificateId) {
  ...
  return rows.find(function(row) {
    return String(row.Certificate_ID || "").trim().toUpperCase() === id;
  }) || null;
}
...
function certificateVerificationFromGateway_(params) {
  ...
  return {
    ...
    data: {
      found: !!record,
      certificate: certificateRegistryPublicRecord_(record)
    }
  };
}
...
function certificateRegistryFromGateway_(params) {
  ...
  if (op === "supersede") {
    ...
    current.Superseded_By = replacementId;
  ...
  if (op === "revoke") {
    ...
    current.Revoked_Reason = certificateRegistryClean_(params.reason, 300) || "Revoked by LAND VIEW administration";
    current.Updated_At = now;
    certificateRegistryWriteRow_(sheet, current._row, current);
    return { success: true, data: { certificate: certificateRegistryPublicRecord_(current) } };
  }
  ...
  if (op === "delete") {
    ...
    current.Deleted_Reason = certificateRegistryClean_(params.reason, 300) || "Withdrawn by LAND VIEW administration";
    current.Updated_At = now;
    certificateRegistryWriteRow_(sheet, current._row, current);
    return { success: true, data: { certificate: certificateRegistryPublicRecord_(current) } };
  }
