...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${service.name} | LAND VIEW Engineers & Architects`,
      description: service.description,
      url,
...
export default async function ServicePage({ params }: PageProps) {
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: "https://landview.com.bd" },
      { "@type": "ListItem", position: 2, name: "Services", item: "https://landview.com.bd/services" },
  ...
  return (
    <main className="lv-service-page">
      ...
      <section className="lv-service-hero">
        <div className="lv-service-wrap">
          <Link className="lv-service-back" href="/services">← ALL SERVICES</Link>
          <div className="lv-service-icon" aria-hidden="true">{service.icon}</div>
          <h1>{service.name}</h1>
      ...
      <section className="lv-service-wrap lv-service-content">
        <article className="lv-service-panel">
          <h2>About this service</h2>
          <p>{service.intro}</p>
          <div className="lv-service-cta">
            <a href="/#contact" className="lv-service-btn lv-service-btn-gold">Start a conversation</a>
            <Link href="/projects" className="lv-service-btn lv-service-btn-outline">View projects</Link>
          </div>
        </article>
        ...
        <aside className="lv-service-panel">
          ...
          <p>LAND VIEW Engineers &amp; Architects is based in Feni and provides design and technical consultancy support for suitable projects in Feni and other parts of Bangladesh.</p>
        </aside>

        <section className="lv-service-panel lv-service-related">
          <h2>Explore more LAND VIEW services</h2>
          <div className="lv-related-grid">
            {related.map((item) => (
              <Link href={`/services/${item.slug}`} className="lv-related-card" key={item.slug}>
                <small>{item.icon} SERVICE</small>
