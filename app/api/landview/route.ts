...
const GET_ACTIONS = new Set([
  "health", "getSession", "getDashboard", "getUsers", "getProjects", "getProject",
  "getProjectEmployees", "getProjectDriveFolder", "getEmployees", "getDocuments", "getSiteVisits",
  "getBillingDashboard", "getProjectBilling", "getBillingRecords", "getPayments", "getInvoices", "getPermissions",
]);
const POST_ACTIONS = new Set([
  "login", "logout", "createUser", "resetUserPassword", "changeOwnPassword", "createProject", "updateProject",
  "deleteProject", "updateProjectEmployees", "createEmployee", "updateEmployee", "deleteEmployee", "createDocument",
  "createSiteVisit", "saveBill", "createBill", "savePayment", "createPayment", "createInvoice", "createPermission",
]);
...
function hostAllowed(request: NextRequest) {
  ...
  if (host === "app.landview.com.bd" || host === "localhost" || host === "127.0.0.1") return true;
...
function originAllowed(request: NextRequest) {
  ...
  try {
    ...
    return host === "app.landview.com.bd" || host === "localhost" || host === "127.0.0.1" || host === process.env.VERCEL_URL?.toLowerCase();
  } catch { return false; }
}
...
async function callBackend(payload: Record<string, unknown>) {
  ...
  catch { throw new Error(/^\s*</.test(text) ? "Apps Script returned HTML instead of JSON." : "Apps Script returned invalid JSON."); }
...
async function handle(request: NextRequest, method: "GET" | "POST") {
  try {
    ...
    if (method === "POST" && !originAllowed(request)) return NextResponse.json({ success: false, error: "Invalid request origin." }, { status: 403 });

    let input: Record<string, unknown> = {};
    if (method === "GET") request.nextUrl.searchParams.forEach((v, k) => { input[k] = v; });
    else {
      try { input = await request.json(); }
      catch { return NextResponse.json({ success: false, error: "Invalid JSON request." }, { status: 400 }); }
    }

    const action = String(input.action || "");
    const allowed = method === "GET" ? GET_ACTIONS : POST_ACTIONS;
    if (!allowed.has(action)) return NextResponse.json({ success: false, error: "Unsupported action." }, { status: 405 });
    ...
    const payload: Record<string, unknown> = { ...input, action, proxySecret: PROXY_SECRET, _clientKey: clientKey(request) };
    ...
    if (json?.success && returnedToken && (action === "login" || action === "changeOwnPassword")) setSessionCookie(out, returnedToken);
    if (action === "logout" || (!json?.success && /unauthorized|session expired/i.test(String(json?.error || json?.message || "")))) clearSessionCookie(out);
    return out;
  } catch (error: any) {
