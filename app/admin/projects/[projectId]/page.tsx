 ...
 return <>
  ...
  <div className="project-detail-grid">
   <section className="card project-summary"><div className="section-title"><div><span>PROJECT PROFILE</span><h2>Overview</h2></div><StatusBadge value={pick(project,["Status","status","Active"],"Active")}/></div>
    <div className="detail-list"><div><span>Client</span><strong>{pick(project,["Client_Name","Client Name"],"—")}</strong></div><div><span>Phone</span><strong>{pick(project,["Phone_Number","Phone Number","Phone"],"—")}</strong></div><div><span>Type</span><strong>{pick(project,["Project_Type","Project Type"],"—")}</strong></div><div><span>Start date</span><strong>{formatDate(project.Start_Date||project["Start Date"])}</strong></div></div>
   </section>
  ...
  {editing&&<form className="card form-card compact" onSubmit={saveEdit}><div className="section-title"><div><span>EDIT</span><h2>Project details</h2></div></div><div className="form-grid">
   ...
   </div><div className="form-actions"><button type="button" onClick={remove} className="btn btn-danger">Delete project</button><button className="btn btn-dark">Save changes</button></div></form>}
