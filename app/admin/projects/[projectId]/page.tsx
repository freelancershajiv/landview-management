...
function financeFromSummary(rows:string[][],projectId:string,updatedAt?:string):LiveFinance|null{
  ...
  const billed=categories.reduce((sum,c)=>sum+c.bill-c.discount,0); const paid=categories.reduce((sum,c)=>sum+c.paid,0); const due=moneyNumber(row[15]);
...
export default function ProjectDetailPage(){
  ...
  const [tab,setTab]=useState<Tab>("overview"); const [project,setProject]=useState<any>(null); const [employees,setEmployees]=useState<any[]>([]); const [assigned,setAssigned]=useState<string[]>([]); const [billing,setBilling]=useState<any>(null); const [liveFinance,setLiveFinance]=useState<LiveFinance|null>(null); const [drive,setDrive]=useState<any>(null); const [serviceFolders,setServiceFolders]=useState<ProjectServiceFolderInfo[]>([]); const [visits,setVisits]=useState<any[]>([]); const [documents,setDocuments]=useState<any[]>([]); const [tasks,setTasks]=useState<any[]>([]); const [folderFiles,setFolderFiles]=useState<Record<string,File|null>>({}); const [uploadingFolder,setUploadingFolder]=useState(""); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [editing,setEditing]=useState(false); const [draft,setDraft]=useState<any>({}); const [savingTeam,setSavingTeam]=useState(false); const [savingProject,setSavingProject]=useState(false);
  ...
  const orderedTasks=useMemo(()=>[...tasks].sort((a,b)=>{
    const at=String(idOf(a,["Task_Title","Task Title","Title"])); const bt=String(idOf(b,["Task_Title","Task Title","Title"]));
    const ai=WORKFLOW_ORDER.indexOf(at); const bi=WORKFLOW_ORDER.indexOf(bt); return (ai<0?999:ai)-(bi<0?999:bi)||at.localeCompare(bt);
  }),[tasks]);
  const completed=orderedTasks.filter(task=>String(idOf(task,["Status","status"])).toLowerCase()==="completed").length;
  ...
  if(loading)return <LoadingState label="Loading project command center..."/>; if(error&&!project)return <ErrorState message={error} onRetry={load}/>;
  const title=idOf(project,["Project_Name","Project Name","Name","Project_Type","Project Type"])||projectId; const billed=liveFinance?.billed??Number(billing?.totalBill||0); const paid=liveFinance?.paid??Number(billing?.totalPaid||0); const due=liveFinance?.due??Number(billing?.due||0); const collection=billed>0?Math.max(0,Math.round(paid/billed*100)):0;
  ...
  return <><style dangerouslySetInnerHTML={{__html:css}}/><div className="pc">
    ...
    </section><div className="pc-two"><section className="pc-panel"><div className="pc-panel-head"><div><span>PROJECT CONTROL</span><h2>Workflow health</h2></div><button className="btn btn-small" onClick={()=>setTab("workflow")}>Open workflow →</button></div><div className="pc-progress"><span style={{width:`${progress}%`}}/></div><div className="pc-progress-copy"><span>{completed} of {orderedTasks.length} done</span><span>{orderedTasks.length-completed} remaining · {progress}%</span></div><div className="pc-list" style={{marginTop:16}}>{orderedTasks.length?orderedTasks.slice(0,5).map((task,i)=>{const stage=String(idOf(task,["Task_Title","Task Title","Title"]));const s=String(idOf(task,["Status","status"])||"Pending");return <div className="pc-row" key={String(task.Task_ID||stage)}><div><strong>{String(i+1).padStart(2,"0")} · {stage}</strong><small>{idOf(task,["Assigned_Employee_ID","Assigned Employee ID"])||"Unassigned"}</small></div><span className={`pc-badge ${s==="Completed"?"green":s==="Blocked"?"red":""}`}>{s}</span></div>}):<div className="pc-empty">No billed services are configured for workflow yet.</div>}</div></section>
    ...
    {tab==="workflow"&&<section className="pc-panel"><div className="pc-panel-head"><div><span>BILLING-DRIVEN DELIVERY</span><h2>Project workflow</h2></div><div className="pc-actions"><button className="btn btn-small" onClick={load}>Sync / Refresh</button></div></div><div className="pc-progress"><span style={{width:`${progress}%`}}/></div><div className="pc-progress-copy"><span>{completed} of {orderedTasks.length} completed</span><span>{progress}% overall</span></div><p className="pc-note">Only services billed to this project are counted. Completing 3 of 6 required services = 50% project workflow.</p><div className="pc-workflow" style={{marginTop:16}}>{orderedTasks.length?orderedTasks.map((task,index)=>{const stage=String(idOf(task,["Task_Title","Task Title","Title"]));return <WorkflowStage key={String(task.Task_ID)} task={task} title={stage} index={index} employees={employees} onSaved={record=>setTasks(rows=>rows.map(row=>String(row.Task_ID)===String(record.Task_ID)?record:row))}/>}):<div className="pc-empty">No workflow services were found in Engineering, Supervision or Other Bills for this project.</div>}</div></section>}
