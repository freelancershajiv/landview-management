...
export default function ProjectDetailPage(){
  ...
  async function load(){
    ...
    try{
      const [p,allEmp,assignedEmp,bill,folder,services,sv,docs,allTasks]=await Promise.all([
        landViewApi.getProject(projectId), landViewApi.getEmployees(), landViewApi.getProjectEmployees(projectId),
        landViewApi.getProjectBilling(projectId), landViewApi.getProjectDriveFolder(projectId), landViewApi.getProjectServiceFolders(projectId),
      ...
      setBilling(bill); setDrive(folder); setServiceFolders(services.folders||[]); setVisits(sv); setDocuments(docs);
  ...
  const due=Number(billing?.due||0), billed=Number(billing?.totalBill||0), paid=Number(billing?.totalPaid||0);
  ...
  return <><style dangerouslySetInnerHTML={{__html:css}}/><div className="pc">
    ...
    <div className="pc-tabs" role="tablist">{tabs.map(([key,label])=><button key={key} className={tab===key?"active":""} onClick={()=>setTab(key)}>{label}{key==="workflow"?` · ${progress}%`:key==="documents"?` · ${documents.length}`:key==="visits"?` · ${visits.length}`:""}</button>)}</div>
    ...
    {tab==="overview"&&<>
      <section className="pc-grid">
        ...
        <div className="pc-stat"><span>Outstanding</span><strong><Money value={due}/></strong><small><Money value={paid}/> received</small></div>
      ...
      <section className="pc-panel"><div className="pc-panel-head"><div><span>COMMERCIAL</span><h2>Project finance</h2></div><button className="btn btn-small" onClick={()=>setTab("finance")}>Details →</button></div><div className="pc-list"><div className="pc-row"><strong>Total billed</strong><strong><Money value={billed}/></strong></div><div className="pc-row"><strong>Total received</strong><strong><Money value={paid}/></strong></div><div className="pc-row"><strong>Outstanding</strong><strong><Money value={due}/></strong></div><div className="pc-row"><strong>Bills / payments</strong><span className="pc-badge">{billing?.bills?.length||0} / {billing?.payments?.length||0}</span></div></div></section></div>
    </>}
    ...
    {tab==="finance"&&<section className="pc-panel"><div className="pc-panel-head"><div><span>FINANCE & ACCOUNTS</span><h2>Commercial position</h2></div><Link className="btn btn-small" href="/admin/finance">Open Finance →</Link></div><section className="pc-grid"><div className="pc-stat"><span>Total billed</span><strong><Money value={billed}/></strong></div><div className="pc-stat"><span>Received</span><strong><Money value={paid}/></strong></div><div className="pc-stat"><span>Outstanding</span><strong><Money value={due}/></strong></div><div className="pc-stat"><span>Collection</span><strong>{billed>0?Math.round(paid/billed*100):0}%</strong></div></section><div className="pc-two" style={{marginTop:14}}><div><h3>Bills</h3><div className="pc-list">{(billing?.bills||[]).slice(0,10).map((r:any,i:number)=><div className="pc-row" key={i}><div><strong>{idOf(r,["Bill_ID","Bill ID","Category","Bill_Category"])||`Bill ${i+1}`}</strong><small>{formatDate(r.Bill_Date||r.Date||r.Created_At)}</small></div><strong><Money value={Number(idOf(r,["Amount","Bill_Amount","Total"])||0)}/></strong></div>)}</div></div><div><h3>Payments</h3><div className="pc-list">{(billing?.payments||[]).slice(0,10).map((r:any,i:number)=><div className="pc-row" key={i}><div><strong>{idOf(r,["Payment_ID","Payment ID","Method","Payment_Method"])||`Payment ${i+1}`}</strong><small>{formatDate(r.Payment_Date||r.Date||r.Created_At)}</small></div><strong><Money value={Number(idOf(r,["Amount","Payment_Amount"])||0)}/></strong></div>)}</div></div></div></section>}
