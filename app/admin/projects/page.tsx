...
function buildProjectsFromDrive(source:Record<string,any>,fileRows:string[][]):{projects:InvoiceProject[];drive:Record<string,DriveProjectInfo>}{
  const files=buildFileListMap(fileRows);const drive:Record<string,DriveProjectInfo>={};const projects:InvoiceProject[]=[];
  Object.entries(source||{}).forEach(([rawId,item])=>{
    const id=normalizeFinanceId(rawId||item?.projectId);if(!id)return;
    const row=files[id]||[];const category=normalizeProjectCategory(item?.category||"Running");const folderName=String(item?.projectFolderName||"").trim();const fileName=String(row[1]||"").trim();
    projects.push({Project_ID:id,Project_Name:fileName||folderDisplayName(folderName,id),Client_Name:fileName||folderDisplayName(folderName,id),Phone_Number:String(row[3]||"").trim(),Location:String(row[2]||"").trim(),Floors:String(row[4]||"").trim(),Project_Type:String(row[5]||"").trim(),Plot_Area:String(row[6]||"").trim(),Status:category});
    drive[id]={found:true,category,projectFolderId:item?.projectFolderId,projectFolderName:folderName,projectFolderUrl:item?.projectFolderUrl,folders:item?.folders||[]};
  });
  projects.sort((a,b)=>Number(b.Project_ID.replace("LV-",""))-Number(a.Project_ID.replace("LV-","")));
...
export default function ProjectsPage(){
  ...
  async function load(){
    ...
    try{
      ...
      const running=buildProjectsFromDrive(runningIndex?.projects||{},fileRows);
      setProjects(running.projects);setDrive(running.drive);setFinance(summary?buildFinanceMap(summary.rows):{});setLoading(false);setBackgroundLoading(true);

      void Promise.all([
        getDriveIndex("Paused").catch(()=>({bulk:true,category:"Paused",projects:{}} as DriveIndexResponse)),
        getDriveIndex("Completed").catch(()=>({bulk:true,category:"Completed",projects:{}} as DriveIndexResponse)),
        landViewApi.getDocuments().catch(()=>[]),
      ]).then(async([pausedIndex,completedIndex,documents])=>{
        ...
        setProjects(current=>mergeProjects(current,[...paused.projects,...completed.projects]));
        setDrive(current=>({...current,...paused.drive,...completed.drive}));
        const map=buildDocumentPreviewMap(documents);
        try{const response=await fetch("/api/public/projects",{cache:"no-store"});const json=await response.json();const publicRows:PublicProjectPreview[]=Array.isArray(json?.data)?json.data:Array.isArray(json)?json:[];publicRows.forEach(p=>{const id=normalizeFinanceId(p.projectId);const cover=imageUrl(p.coverImageUrl);if(id&&cover&&!map[id])map[id]=cover;});}catch{}
        setPreviews(map);
      }).finally(()=>setBackgroundLoading(false));
    }catch(e:any){setError(e?.message||"Could not load Running projects from LV Project Files.");setLoading(false);}
  }
