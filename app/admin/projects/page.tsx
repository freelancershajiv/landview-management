...
function buildProjectsFromFileList(rows:string[][],statusMap:Record<string,ProjectCategory>):InvoiceProject[]{return rows.map(row=>{const id=normalizeFinanceId(row[0]);const name=String(row[1]||"").trim();if(!id||!name)return null;return{Project_ID:id,Project_Name:name,Client_Name:name,Phone_Number:String(row[3]||"").trim(),Location:String(row[2]||"").trim(),Floors:String(row[4]||"").trim(),Project_Type:String(row[5]||"").trim(),Plot_Area:String(row[6]||"").trim(),Status:statusMap[id]||"Running"};}).filter((project):project is InvoiceProject=>Boolean(project)).sort((a,b)=>Number(b.Project_ID.replace("LV-",""))-Number(a.Project_ID.replace("LV-","")));}
