...
const initial = {
  ...
  Public_Display: "FALSE", Public_Project_Title: "", Public_Description: "", Project_Category: "", Project_Area: "", Number_of_Stories: "", Cover_Image_URL: "", Gallery_Images: "", Public_Services: "", Completion_Year: "", Public_Display_Order: "",
};
...
export default function NewProjectPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    setSaving(true);
    setError("");
    setProgress("Creating project and Drive structure...");
    try {
      const result = await landViewApi.createProject(form);
      const id = String(result.Project_ID || form.Project_ID || "").trim();
      ...
      if (total) {
        await uploadQueuedProjectFiles(id, queue, (done, count, fileName) => {
          setProgress(done >= count ? "Uploads complete." : `Uploading ${done + 1} of ${count}: ${fileName}`);
        });
      }
      ...
      if (account?.created && account.temporaryPassword) {
        ...
        router.push(`/admin/projects/${encodeURIComponent(id)}`);
      }
    } catch (e: any) {
  ...
  return <>
    <PageHeader eyebrow="PROJECT CONTROL" title="Create project" description="Create the project, client record, Drive structure and project-service files in one workflow." action={<Link href="/admin/projects" className="btn btn-light">← Projects</Link>} />
    ...
    <form onSubmit={submit} className="card form-card">
      ...
      {progress && <div className="notice"><strong>Working</strong><span>{progress}</span></div>}

      <div className="form-section"><div><span>01</span><h2>Project identity</h2><p>Project ID can be entered manually. If left blank, Code.gs generates an LV- ID.</p></div><div className="form-grid">
      ...
      <div className="form-section"><div><span>02</span><h2>Client & location</h2><p>Record the client contact information and project location.</p></div><div className="form-grid">
      ...
      <div className="form-section"><div><span>04</span><h2>Project folders & files</h2><p>All 11 service folders are created automatically in the project Drive folder. Select files now and they will upload after the project is created.</p></div>
        <QueuedProjectFiles queue={queue} onChange={setQueue} />
        <p className="muted">Queued files: {totalQueuedFiles(queue)}. App uploads are limited to 3 MB per file; larger files can be uploaded directly through the Drive folder from the project page.</p>
      </div>

      <div className="form-section"><div><span>05</span><h2>Public portfolio</h2><p>Only projects explicitly enabled here should appear on the public LAND VIEW portfolio.</p></div><div className="form-grid">
