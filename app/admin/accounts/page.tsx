...
export default function AccountsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    Promise.all([landViewApi.getPayments(), landViewApi.getErpRecords("expenses")])
  ...
  const ledgerRows = useMemo<RunningRow[]>(() => {
    let balance = 0;
    return [...incomeRows, ...postedExpenses]
  ...
  return <div className="accounts-page">
    ...
    <div className="accounts-note">Canonical cashbook: financially effective Payments are credits; only approved/posted Expenses are debits. Pending expenses remain visible but do not reduce the ledger balance.</div>
    ...
    {mode === "dashboard" && <section className="accounts-report">
      <div className="accounts-card"><span>Posted transactions</span><strong>{ledgerRows.length}</strong><p>{incomeRows.length} effective income records · {postedExpenses.length} posted expense records</p></div>
      <div className="accounts-card"><span>Canonical source</span><strong>Payments + Expenses</strong><p>The web ledger now uses the same canonical records used by billing and approvals. The retired Accounting Income/Accounting Expenses compatibility sheets are no longer requested.</p></div>
    </section>}
    ...
    {mode === "ledger" && <>
      <section className="ledger-summary">
        <div className="accounts-card"><span>Total credits</span><strong className="accounts-income">{money(totals.income)}</strong></div>
        <div className="accounts-card"><span>Total debits</span><strong className="accounts-expense">{money(totals.approvedExpense)}</strong></div>
        <div className="accounts-card"><span>Running balance</span><strong>{money(totals.net)}</strong></div>
      </section>
      <section className="accounts-panel">
        <div className="accounts-toolbar"><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search transaction, project, category, party or reference..."/><select value={year} onChange={(event) => setYear(event.target.value)}><option value="">All years</option>{years.map((item) => <option key={item}>{item}</option>)}</select></div>
        {loading ? <div className="accounts-empty">Loading canonical ledger…</div> : filteredLedger.length === 0 ? <div className="accounts-empty">No matching posted transactions.</div> : <div className="accounts-table"><table><thead><tr><th>Date</th><th>Transaction / Project</th><th>Account</th><th>Details</th><th className="accounts-num">Debit</th><th className="accounts-num">Credit</th><th className="accounts-num">Balance</th></tr></thead><tbody>{filteredLedger.map((row) => <tr key={`${row.type}-${row.id}`}><td>{displayDate(row.date)}</td><td><strong>{row.id}</strong><br/><span style={{color:"#89959e"}}>{row.projectId || "—"}</span></td><td>{row.account || row.method || "—"}</td><td>{row.description}<br/><span style={{color:"#89959e"}}>{row.party || row.category}{row.reference ? ` · ${row.reference}` : ""}</span></td><td className="accounts-expense accounts-num">{row.debit ? money(row.debit) : "—"}</td><td className="accounts-income accounts-num">{row.credit ? money(row.credit) : "—"}</td><td className="accounts-balance accounts-num">{money(row.balance)}</td></tr>)}</tbody></table></div>}
      </section>
      <div style={{marginTop:10,color:"#7f8b94",fontSize:11}}>Running balances are calculated from the full posted ledger before search/year filtering, so filtering does not change historical balances.</div>
    </>}
