...
function summarize(rows: LedgerRow[]): Summary {
  const effectiveIncome = rows.filter(
    (row) => row.type === "Income" && !["pending", "rejected", "returned"].includes(row.status.toLowerCase()),
  );
  const approvedExpense = rows.filter(
    (row) => row.type === "Expense" && row.status.toLowerCase() === "approved",
  );
...
export default function AccountsLedgerPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((err: unknown) => {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Could not load the accounts ledger.");
        }
      })
      .finally(() => {
  ...
  const allRows = useMemo(
    () =>
      [...toIncomeRows(incomeData), ...toExpenseRows(expenseData)].sort(
  ...
  const categoryBreakdown = useMemo(() => {
    ...
    filteredRows.forEach((row) => {
      ...
      if (
        row.type === "Income" &&
        !["pending", "rejected", "returned"].includes(row.status.toLowerCase())
      ) {
        item.income += row.amount;
      }
      if (row.type === "Expense" && row.status.toLowerCase() === "approved") {
  ...
  return (
    <main className="accounts-page">
      <style>{`
        ...
        .ac-panel{overflow:hidden;border:1px solid #28343d;border-radius:11px;background:#0f171e}.ac-panel-head{display:flex;justify-content:space-between;gap:15px;align-items:center;padding:13px 14px;border-bottom:1px solid #28343d}.ac-panel-head h2{margin:0;font-size:15px}.ac-panel-head span{color:#74818a;font-size:9px}.ac-table-wrap{overflow:auto}.ac-table{width:100%;border-collapse:collapse;min-width:1120px}.ac-table th{position:sticky;top:0;background:#151f27;color:#7c8992;text-align:left;font-size:8px;text-transform:uppercase;letter-spacing:.06em;padding:9px 11px;border-bottom:1px solid #33404a}.ac-table td{padding:11px;border-bottom:1px solid #222d35;font-size:10px;vertical-align:top}.ac-table tbody tr:hover td{background:#121c23}.ac-table .detail{max-width:420px}.ac-table .detail strong{display:block;font-size:11px}.ac-sub{display:block;margin-top:4px;color:#6f7d87;font-size:8px;line-height:1.5}.ac-project{font-weight:800;color:#e07b78}.ac-type{display:inline-flex;margin-top:5px;padding:3px 6px;border-radius:999px;font-size:8px;font-weight:900}.ac-type.income{background:#153523;color:#93deb0}.ac-type.expense{background:#3a1e20;color:#ffaaa6}.ac-person{min-width:155px}.ac-person strong{display:block;color:#f2f5f7;font-size:11px;line-height:1.35}.ac-person-role{display:block;margin-bottom:3px;color:#74818a;font-size:8px;font-weight:800;text-transform:uppercase;letter-spacing:.06em}.ac-status{display:inline-flex;padding:3px 6px;border-radius:999px;background:#202b33;color:#bac4ca;font-size:8px}.ac-status.pending{background:#3b321c;color:#efd28e}.ac-status.rejected,.ac-status.returned{background:#3b1f21;color:#ffaca8}.ac-amount{display:block;font-size:12px;font-weight:900;white-space:nowrap}.ac-amount.income{color:#8bd9a7}.ac-amount.expense{color:#ffaaa5}.ac-amount-cell{text-align:right;min-width:125px}.ac-table th:last-child{text-align:right}.ac-empty{padding:34px;text-align:center;color:#74818a;font-size:10px}.ac-footer{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 13px;color:#6f7c85;font-size:9px}.ac-footer div{display:flex;gap:6px}.ac-footer button{border:1px solid #33404a;border-radius:6px;background:#151f27;color:#d9e0e4;padding:6px 9px;font-size:9px;cursor:pointer}.ac-footer button:disabled{opacity:.35}
        ...
        @media print{.ac-actions,.ac-toolbar,.ac-advanced,.ac-tabs,.ac-footer{display:none!important}.accounts-page{color:#111}.ac-card,.ac-panel,.ac-report{background:#fff;border-color:#ccc}.ac-table th{background:#eee;color:#333}.ac-table td{color:#111}.ac-sub,.ac-person-role{color:#555}.ac-person strong{color:#111}.ac-print{display:block}}
      `}</style>
      ...
      <header className="ac-head">
        <div>
          ...
          <p>
            Review money in, approved money out, who received each payment, and items still waiting for approval.
          </p>
        </div>
        <div className="ac-actions">
          <button className="ac-btn" disabled={loading} onClick={() => setRevision((value) => value + 1)}>
            ↻ Refresh
          </button>
          <button
            className="ac-btn"
          ...
          <Link className="ac-btn primary" href="/admin/accounts/entry">
            + New transaction
          </Link>
        </div>
      </header>
      ...
      <section className="ac-summary">
        <article className="ac-card income">
          <span>Income</span>
          <strong>{money(summary.income)}</strong>
          <small>{summary.incomeCount.toLocaleString("en-BD")} effective receipts</small>
        </article>
        <article className="ac-card expense">
          <span>Approved expenses</span>
          <strong>{money(summary.expense)}</strong>
          <small>{summary.expenseCount.toLocaleString("en-BD")} approved records</small>
        </article>
        ...
        <article className="ac-card pending">
          <span>Waiting approval</span>
          <strong>{money(summary.pending)}</strong>
          <small>{summary.pendingCount.toLocaleString("en-BD")} pending transactions</small>
        </article>
      </section>
      ...
      <section className="ac-toolbar">
        <input
          ...
          type="search"
          value={filters.query}
          placeholder="Search ID, project, description, person, reference…"
          onChange={(event) => patchFilter("query", event.target.value)}
        />
        <select
          className="ac-year"
          value={filters.year}
          onChange={(event) => patchFilter("year", event.target.value)}
        >
        ...
        <button className="ac-btn ac-filter-toggle" onClick={() => setShowAdvanced((value) => !value)}>
          Filters
          {activeFilterCount > (filters.query ? 1 : 0) + (filters.year ? 1 : 0) ? (
      ...
      {mode === "reports" ? (
        <section className="ac-report-grid">
          <article className="ac-report">
            ...
            {yearly.map((item) => (
              <div className="ac-year-row" key={item.year}>
                <b>{item.year}</b>
                <span className="positive">{money(item.income)}</span>
                <span className="negative">{money(item.expense)}</span>
          ...
          <article className="ac-report" style={{ gridColumn: "1 / -1" }}>
            ...
            <div className="ac-category-row">
              <b>Filtered view</b>
              <span className="positive">{money(summary.income)}</span>
              <span className="negative">{money(summary.expense)}</span>
              <span>{money(summary.net)}</span>
            </div>
            <div className="ac-category-row">
              <b>All records</b>
              <span className="positive">{money(overall.income)}</span>
              <span className="negative">{money(overall.expense)}</span>
              <span>{money(overall.net)}</span>
            </div>
          </article>
        </section>
      ) : (
        <section className="ac-panel">
          <div className="ac-panel-head">
            ...
            <span>{filteredRows.length.toLocaleString("en-BD")} matching records</span>
          </div>

          {loading ? (
            <div className="ac-loading">Loading accounts…</div>
          ) : (
            <>
                      ...
                      return (
                        <tr key={`${row.type}-${row.id}`}>
                          <td>
                            ...
                            <span className={`ac-type ${row.type.toLowerCase()}`}>{row.type}</span>
                          </td>
                          <td className="detail">
                          ...
                          <td className="ac-person">
                            <span className="ac-person-role">
                              {row.type === "Expense" ? "Paid to" : "Received from"}
                            </span>
                            <strong>{row.person || "Not specified"}</strong>
                          </td>
                          <td>
                            <span className="ac-project">{row.fileId || "—"}</span>
                            {row.projectName && <span className="ac-sub">{row.projectName}</span>}
                          </td>
                          <td>{row.category}</td>
                          <td>
                            <span className={`ac-status ${statusClass}`}>{row.status}</span>
                          </td>
                          <td className="ac-amount-cell">
                            <span className={`ac-amount ${row.type.toLowerCase()}`}>
                              {row.type === "Income" ? "+" : "−"}
                              {money(row.amount)}
                            </span>
                            <span className="ac-sub">{row.person || "No person recorded"}</span>
                          </td>
                        </tr>
                      );
                    })}
              ...
              <div className="ac-footer">
                <span>
                  Showing {shown.length ? currentPage * 40 + 1 : 0}–
                  {Math.min((currentPage + 1) * 40, filteredRows.length)} of {filteredRows.length}
                </span>
                <div>
                  <button
