...
export default function BillingPage() {
  ...
  const openProjects = useMemo(() => projects
    ...
    .sort((a, b) => a.id.localeCompare(b.id, undefined, { numeric: true })), [projects]);

  const rows = useMemo(() => {
    const term = query.trim().toLowerCase();
    return (book?.projects || [])
      .filter((row) => !term || [row.projectId, row.projectName, row.clientName].join(" ").toLowerCase().includes(term))
  ...
  const paymentProjects = useMemo(() => (book?.projects || [])
    ...
    .sort((a, b) => a.id.localeCompare(b.id, undefined, { numeric: true })), [book]);
  ...
  async function saveBill() {
    ...
    try {
      await landViewApi.saveBill({
        ...
        Idempotency_Key: `web-bill-${crypto.randomUUID()}`,
      });
      setMessage("Bill added successfully.");
      setBillOpen(false);
      setRevision((value) => value + 1);
    } catch (err) {
      setMessage(err instanceof Error ? err.message : "Could not add bill.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function savePayment() {
    ...
    try {
      await landViewApi.savePayment({
        ...
        Idempotency_Key: `web-payment-${crypto.randomUUID()}`,
      });
      setMessage("Payment recorded and sent for the configured approval/accounting flow.");
  ...
  return <div className="billing-page">
    ...
    <header className="billing-head">
      ...
      <div className="billing-actions">
        <button className="billing-btn primary" type="button" onClick={() => startBill()}>+ Add Bill</button>
        <Link className="billing-btn" href="/admin/finance/invoices">Generate Invoice</Link>
        <button className="billing-btn" type="button" onClick={() => startPayment()}>+ Add Payment</button>
        <Link className="billing-btn" href="/admin/accounts">Accounts</Link>
      </div>
    </header>

    {message && <div className={`billing-note ${/could not|cannot|choose|valid|no outstanding/i.test(message) ? "error" : ""}`}>{message}</div>}
    {error && <div className="billing-note error">{error}</div>}

    <section className="billing-metrics">
      <div className="billing-card"><span>Net billed</span><strong>{money(totals.billed)}</strong></div>
    ...
    <section className="billing-cats">
      {BILL_CATEGORIES.map((category) => {
        ...
        return <div className="billing-card billing-cat" key={category}>
          <h3>{category}</h3>
          <dl><dt>Billed</dt><dd>{money(totalsForCategory?.billed || 0)}</dd><dt>Collected</dt><dd>{money(totalsForCategory?.paid || 0)}</dd><dt>Due</dt><dd>{money(totalsForCategory?.due || 0)}</dd></dl>
    ...
    <section className="billing-panel">
      <div className="billing-toolbar"><div><strong>Project receivables</strong><div className="billing-help">Calculated directly from canonical Bills and approved Payments.</div></div><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search File ID, project or client..." /></div>
      {loading ? <div className="billing-empty">Loading canonical billing data…</div> : rows.length === 0 ? <div className="billing-empty">No matching billing records.</div> : <div className="billing-table"><table><thead><tr><th>File ID</th><th>Project / Client</th><th>Billed</th><th>Collected</th><th>Due</th><th>Project status</th><th>Invoice</th></tr></thead><tbody>{rows.map((row) => {
        const project = projectById.get(normalizeProjectId(row.projectId));
        const projectStatus = text(field(project || {}, ["Status", "Project_Status", "Project Status"])) || "—";
        return <tr key={row.projectId}><td><strong>{normalizeProjectId(row.projectId)}</strong></td><td><strong>{row.projectName || row.projectId}</strong><br/><span className="billing-help">{row.clientName || "—"}</span></td><td>{money(row.billed)}</td><td>{money(row.paid)}</td><td className={row.due > 0.009 ? "billing-due" : "billing-ok"}>{money(row.due)}</td><td>{projectStatus}</td><td><Link className="billing-btn" href={`/admin/finance/invoices?fileId=${encodeURIComponent(normalizeProjectId(row.projectId))}`}>Open</Link></td></tr>;
      })}</tbody></table></div>}
    </section>

    {billOpen && <div className="billing-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && !saving) setBillOpen(false); }}><div className="billing-modal" role="dialog" aria-modal="true" aria-label="Add bill"><h2>Add Bill</h2><div className="billing-grid">
      ...
      <div className="billing-field"><label>Amount (BDT)</label><input inputMode="decimal" value={billForm.amount} onChange={(event) => setBillForm((form) => ({ ...form, amount: event.target.value }))}/></div>
    ...
    </div><div className="billing-modal-actions"><button className="billing-btn" disabled={saving} type="button" onClick={() => setBillOpen(false)}>Cancel</button><button className="billing-btn primary" disabled={saving} type="button" onClick={saveBill}>{saving ? "Saving…" : "Save Bill"}</button></div></div></div>}

    {paymentOpen && <div className="billing-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && !saving) setPaymentOpen(false); }}><div className="billing-modal" role="dialog" aria-modal="true" aria-label="Add payment"><h2>Add Payment</h2><div className="billing-grid">
    ...
    </div><div className="billing-modal-actions"><button className="billing-btn" disabled={saving} type="button" onClick={() => setPaymentOpen(false)}>Cancel</button><button className="billing-btn primary" disabled={saving} type="button" onClick={savePayment}>{saving ? "Saving…" : "Save Payment"}</button></div></div></div>}
  </div>;
}
