...
type CategoryDueFilter = "all" | "eb" | "db" | "sb" | "ob";
...
const CATEGORY_DUE_FILTERS = [
  ...
  { key: "db", label: "DB Due", category: "Design Books" },
...
function canonicalCategoryKey(key: string, category: BillCategory) {
  ...
  if (category === "Design Books") return normalized === "design books" || normalized === "design book";
...
export default function BillingPage() {
  ...
  const billingCounts = useMemo(() => {
    ...
    return {
      ...
      db: billingProjects.filter((row) => projectCategory(row, "Design Books").due > 0.009).length,
  ...
  return <div className="billing-page">
    ...
    <section className="billing-panel">
      ...
      {loading ? <div className="billing-empty">Loading canonical billing and write-off data…</div> : rows.length === 0 ? <div className="billing-empty">{query.trim() ? `No matching ${filterLabel()} records.` : `No ${filterLabel()} records.`}</div> : <div className="billing-table"><table>
        ...
        <tbody>{rows.map((row) => {
          const eb = projectCategory(row, "Engineering Bill"), db = projectCategory(row, "Design Books"), sb = projectCategory(row, "Supervision Bill"), ob = projectCategory(row, "Other Services Bill");
          ...
          return <tr key={row.projectId}>
            ...
            <td><div className="billing-breakdown">{[["EB",eb],["DB",db],["SB",sb],["OB",ob]].map(([label, values]) => { const v = values as CategoryNumbers; return <div className={`billing-breakdown-line ${v.due > 0.009 ? "has-due" : ""}`} key={label as string}><span>{label as string}</span><strong>{money(v.due)} due</strong><em>{v.writtenOff > 0.009 ? `${money(v.writtenOff)} W/O` : ""}</em></div>; })}</div></td>
