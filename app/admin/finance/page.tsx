...
export default function BillingPage() {
  ...
  return <div className="billing-page">
    ...
    <section className="billing-panel"><div className="billing-toolbar"><div><strong>Project receivables</strong><div className="billing-help">Due projects are shown by default. Billing status is calculated from canonical Bills and approved Payments.</div></div><div className="billing-toolbar-right"><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search File ID, project or client..."/><div className="billing-filter-group" role="group" aria-label="Billing status filter"><button type="button" className={`billing-filter ${listFilter === "due" ? "active" : ""}`} aria-pressed={listFilter === "due"} onClick={() => setListFilter("due")}>Due · {billingCounts.due}</button><button type="button" className={`billing-filter ${listFilter === "paid" ? "active" : ""}`} aria-pressed={listFilter === "paid"} onClick={() => setListFilter("paid")}>Paid · {billingCounts.paid}</button></div></div></div>{loading ? <div className="billing-empty">Loading canonical billing data…</div> : rows.length === 0 ? <div className="billing-empty">{query.trim() ? `No matching ${listFilter === "due" ? "due" : "fully paid"} billing records.` : `No ${listFilter === "due" ? "due" : "fully paid"} billing records.`}</div> : <div className="billing-table"><table><thead><tr><th>File ID</th><th>Project / Client</th><th>Billed</th><th>Collected</th><th>Due</th><th>Billing status</th><th>Invoice</th></tr></thead><tbody>{rows.map((row) => { const isDue = row.due > 0.009; return <tr key={row.projectId}><td><strong>{normalizeProjectId(row.projectId)}</strong></td><td><strong>{row.projectName || row.projectId}</strong><br/><span className="billing-help">{row.clientName || "—"}</span></td><td>{money(row.billed)}</td><td>{money(row.paid)}</td><td className={isDue ? "billing-due" : "billing-ok"}>{money(row.due)}</td><td><span className={`billing-status ${isDue ? "due" : "paid"}`}>{isDue ? "Due" : "Full Paid"}</span></td><td><Link className="billing-btn" href={`/admin/finance/invoices?fileId=${encodeURIComponent(normalizeProjectId(row.projectId))}`}>Open</Link></td></tr>; })}</tbody></table></div>}</section>
