...
function categoryKey(projectId: string, category: string) { return `${normalizeProjectId(projectId)}::${normalizeStatus(category)}`; }
function projectCategory(project: AdjustedProject | undefined, category: BillCategory): CategoryNumbers {
  ...
  return match?.[1] || { gross: 0, discount: 0, billed: 0, paid: 0, writtenOff: 0, due: 0 };
}
...
export default function BillingPage() {
  ...
  const adjustedProjects = useMemo<AdjustedProject[]>(() => (book?.projects || []).map((row) => {
    ...
    for (const [key, raw] of Object.entries(row.categories || {})) {
      const canonical = BILL_CATEGORIES.find((category) => canonicalCategoryKey(key, category)) || "Engineering Bill";
      const rawDue = Math.max(0, amount(raw.billed) - amount(raw.paid));
      const activeWriteOff = Math.min(rawDue, writeOffBalances.get(categoryKey(row.projectId, canonical)) || 0);
      categories[key] = { gross: amount(raw.gross), discount: amount(raw.discount), billed: amount(raw.billed), paid: amount(raw.paid), writtenOff: activeWriteOff, due: Math.max(0, rawDue - activeWriteOff) };
    }
    for (const category of BILL_CATEGORIES) {
      if (!Object.keys(categories).some((key) => canonicalCategoryKey(key, category))) {
        const active = writeOffBalances.get(categoryKey(row.projectId, category)) || 0;
        if (active > 0) categories[category] = { gross: 0, discount: 0, billed: 0, paid: 0, writtenOff: active, due: 0 };
      }
    }
  ...
  return <div className="billing-page">
    ...
    <section className="billing-panel">
      ...
      {loading ? <div className="billing-empty">Loading canonical billing and write-off data…</div> : rows.length === 0 ? <div className="billing-empty">{query.trim() ? `No matching ${filterLabel()} records.` : `No ${filterLabel()} records.`}</div> : <div className="billing-table"><table>
        <thead><tr><th>File ID</th><th>Project / Client</th><th>Gross</th><th>Discount</th><th>Net billed</th><th>Collected</th><th>Written off</th><th>Collectible due</th><th>Category breakdown</th><th>Status</th><th>Invoice</th></tr></thead>
        <tbody>{rows.map((row) => {
          ...
          return <tr key={row.projectId}>
            ...
            <td><span className={`billing-status ${statusClass}`}>{status}</span></td><td><Link className="billing-btn" href={`/admin/finance/invoices?fileId=${encodeURIComponent(normalizeProjectId(row.projectId))}`}>Open</Link></td>
          </tr>;
        })}</tbody>
      </table></div>}
    </section>
    ...
    {billOpen && <Modal title="Add Bill" intro="Create a bill. A bill discount reduces the bill itself; it is different from a later write-off." error={modalError} saving={saving} onClose={() => setBillOpen(false)} onSave={saveBill} saveLabel="Save Bill">
      <div className="billing-field full"><label>Project</label><select value={billForm.projectId} onChange={(e) => setBillForm((f) => ({ ...f, projectId: e.target.value }))}><option value="">Choose active project</option>{openProjects.map((p) => <option key={p.id} value={p.id}>{p.id} — {p.projectName}{p.clientName ? ` — ${p.clientName}` : ""}</option>)}</select></div>
