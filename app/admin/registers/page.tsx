...
const BOOK_TYPES=["Architectural & Structural Design Book","A4 Site Book","3D Design Book","Electrical Design Book","Plumbing Design Book","Soil Test Book","Digital Survey Report","Municipality Approval Sheet"];
...
function fileNumber(value:unknown){const match=String(value??"").match(/\d+/);return match?Number(match[0]):-1;}
function displayFileId(value:unknown){const raw=String(value??"").trim();const match=raw.match(/\d+/);if(!match)return raw;return "LV-"+String(Number(match[0])).padStart(3,"0");}
...
export default function RegistersPage(){
  ...
  async function load(){
    ...
    try{
      ...
      setProjects([...(json.data.projects||[])].sort(function(a:Project,b:Project){const diff=fileNumber(b.project_code)-fileNumber(a.project_code);return diff!==0?diff:String(b.project_code).localeCompare(String(a.project_code));}));setDocuments(json.data.documents||[]);setBooks(json.data.books||[]);
  ...
  const docRows=useMemo(function(){
    ...
    return documents.filter(function(row){
      if(term&&!searchText([row.register_code,row.project_code,displayFileId(row.project_code),row.client_name,row.phone_number,row.document_name,row.document_type,row.collection_status,row.return_status]).includes(term))return false;
  ...
  const bookRows=useMemo(function(){
    ...
    return books.filter(function(row){
      if(term&&!searchText([row.book_code,row.project_code,displayFileId(row.project_code),row.client_name,row.phone_number,row.book_type,row.printer_name,row.printing_status,row.delivery_status]).includes(term))return false;
  ...
  function projectLabel(p:Project){return[displayFileId(p.project_code),p.client_name||p.project_name,p.phone_number].filter(Boolean).join(" · ");}
  ...
  return <div className="register-page">
    ...
    {modal&&<div className="register-modal-bg" onMouseDown={function(e){if(e.target===e.currentTarget&&!saving)setModal(false);}}><form className="register-modal" onSubmit={save}>
      ...
      {tab==="documents"?<div className="form-grid">
        ...
        <div className="form-field span2"><label>Book Type</label><select required value={BOOK_TYPES.includes(bookForm.book_type)?bookForm.book_type:"Another"} onChange={function(e){setBookForm(Object.assign({},bookForm,{book_type:e.target.value==="Another"?"":e.target.value}));}}>{BOOK_TYPES.map(function(x){return <option key={x} value={x}>{x}</option>;})}<option value="Another">Another</option></select>{!BOOK_TYPES.includes(bookForm.book_type)&&<input required placeholder="Type the book name" value={bookForm.book_type} onChange={function(e){setBookForm(Object.assign({},bookForm,{book_type:e.target.value}));}}/>}</div>
