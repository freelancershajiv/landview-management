...
function buildStatus(incomeData: FinanceSheetData | null, expenseData: FinanceSheetData | null): SeptemberStatus {
  ...
  const monthlyNet = cleanIncome - approvedExpense - pendingExpense;
...
export default function AccountsStatusPanel() {
  ...
  const closingNegative = status.projectedNet < 0;
  const monthPositive = status.monthlyNet >= 0;
        ...
        <>
          <div className="asc-grid">
            <article className="asc-card opening">
              <span className="asc-label">31 Aug opening</span>
              <strong className="asc-value">{money(status.openingBalance)}</strong>
              <span className="asc-meta">Reconciled balance carried into September</span>
            </article>
            <article className="asc-card income">
              ...
              <span className="asc-meta">Management-confirmed total after duplicate control</span>
            </article>
            <article className="asc-card approved">
              ...
              <span className="asc-meta">{status.approvedCount} unique approved September entries</span>
            </article>
            <article className="asc-card pending">
              ...
              <span className="asc-meta">{status.pendingCount} unique entries awaiting approval</span>
            </article>
            <article className={`asc-card month ${monthPositive ? "" : "negative"}`}>
              <span className="asc-label">September net result</span>
              <strong className="asc-value">{money(status.monthlyNet)}</strong>
              <span className="asc-meta">Income minus all unique September expenses</span>
            </article>
            <article className={`asc-card net ${closingNegative ? "" : "positive"}`}>
              <span className="asc-label">Closing position</span>
              <strong className="asc-value">{money(status.projectedNet)}</strong>
              <span className="asc-meta">Opening balance plus September net result</span>
            </article>
          </div>
          <div className="asc-flags">
            <span className="asc-flag"><strong>September formula</strong> income − approved expenses − pending expenses.</span>
            <span className="asc-flag"><strong>Closing formula</strong> 31 Aug opening + September net result.</span>
