/* LAND VIEW — PUBLIC LIVE BILLING VERIFICATION
 *
 * Read-only backend for the permanent project QR.
 * Security model:
 * - authorizeProxyRequest_ still requires the server-side proxy secret.
 * - getPublicBillingVerification does NOT require a LAND VIEW user session.
 * - only billing totals and the client/project display name are returned.
 * - no phone number, address, edit operation, payment mutation or internal IDs.
 */
...
function publicBillingCategory_(name, billRows, paymentRows, discount) {
  var gross = (billRows || []).reduce(function(sum, row) {
    return sum + publicBillingNumber_(row[4]);
  }, 0);
  var paid = (paymentRows || []).reduce(function(sum, row) {
    return sum + publicBillingNumber_(row[3]);
  }, 0);
  ...
  return {
    ...
    hasData: (billRows || []).length > 0 || (paymentRows || []).length > 0 ||
      Math.abs(gross) > 0.009 || Math.abs(safeDiscount) > 0.009 ||
      Math.abs(paid) > 0.009 || Math.abs(due) > 0.009
  };
}
...
function getPublicBillingVerification(params) {
  ...
  if (!fileRows.length && !summaryRows.length) {
    throw new Error("Project billing record was not found.");
  }
  ...
  // These indexes intentionally mirror lib/sheet-invoices.ts so the public QR
  // and the printed invoice calculate discounts from the same Summary cells.
